...
**해결**
요약 작업은 먼저 PostgreSQL의 SummaryJob 테이블에 PENDING 상태로 저장되도록 했습니다. Dispatcher는 PENDING 또는 재시도 가능한 RETRY_WAIT Job을 조회하고, 이미 Queue에 넘겼지만 아직 완료되지 않은 RUNNING Job 수를 기준으로 max_inflight 한도 내에서 남은 slot만큼만 Celery Queue에 전달하도록 변경했습니다.

Queue에 바로 전달되지 않은 Job은 메모리가 아니라 PostgreSQL에 PENDING / RETRY_WAIT 상태로 남아 있다가 다음 Dispatcher 실행 때 다시 처리 대상이 됩니다.
...
**문제**
주식 검색 API에서 검색 결과를 생성하는 과정에서 쿼리 수가 과도하게 증가하는 문제가 발생했고 전체 API 응답 시간도 길어지는 문제가 있었습니다.

**원인 파악**
검색 결과 목록을 생성하는 과정에서 객체마다 추가 쿼리가 발생하는 N+1 문제가 있었습니다.  
N+1 문제를 제거한 뒤에도 Django Debug Toolbar로 확인한 SQL 실행 시간 대비 전체 응답 시간이 길어 페이지네이션 없이 많은 객체를 한 번에 반환하는 구조가 응답 크기와 직렬화 비용을 키우는 것으로 판단했습니다.
...
**결과**
N+1 문제를 재현한 테스트 기준으로 쿼리 수를 **1001개 → 1**개로 줄이고, 테스트 시간을 **0.26초 → 0.03**초 수준으로 개선했습니다.
이후 페이지네이션 적용으로 실제 검색 API 응답 시간을 **1750ms → 37ms**로 개선했습니다.
...
**결과**
Worker 장애나 작업 중단이 발생해도 Job이 영구적으로 멈춰 있지 않고 재처리될 수 있는 구조를 만들었습니다.